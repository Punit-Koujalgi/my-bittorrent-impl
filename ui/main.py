...
def make_binary_executable():
    ...
    if binary_path.exists() and binary_path.stat().st_size < 1000:
        print("Detected LFS pointer, attempting to pull real binary...")
        from huggingface_hub import hf_hub_download
        
        repo_id = os.getenv("SPACE_ID", "pkoujalgi/my-bittorrent-impl") 
        if repo_id:
            try:
                os.chmod(binary_path, stat.S_IWRITE | stat.S_IREAD)
                downloaded_path = hf_hub_download(
                    repo_id=repo_id, 
                    filename="ui/bittorrent",
                    repo_type="space"
                )
                # Copy the real binary over the pointer
                shutil.copy(downloaded_path, binary_path)
                print("Successfully replaced pointer with real binary.")
            except Exception as e:
                print(f"❌ Failed to download binary: {str(e)}")
                return False
...
def download_content(torrent_file_path=None, magnet_link=None, progress=gr.Progress()):
    ...
    base_name = "downloaded_file.gif"
    if torrent_file_path:
        base_name = Path(torrent_file_path).stem

    downloaded_file = ui_dir / base_name  # Use absolute path
...
with gr.Blocks(css=custom_css, title="BitTorrent Client", fill_width=True) as demo:
        ...
        with gr.Column(scale=1):
            ...
            with gr.Group():
                ...
                preset_magnet = gr.Dropdown(
                    ...
                    label="📋 Some Magnet Links.. (safe and widely-used)"
                )
            ...
            with gr.Group():
                ...
                preset_torrent = gr.Dropdown(
                    ...
                    label="📋 Some Torrent Files.. (safe and widely-used)"
                )
